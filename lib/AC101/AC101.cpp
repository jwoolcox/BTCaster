...
esp_err_t AC101::InitI2C(int DATA_IO, int CLK_IO)
{
    i2c_port_t i2c_master_port = (i2c_port_t)I2C_MASTER_NUM;
    i2c_config_t conf;
    conf.mode = I2C_MODE_MASTER;
    conf.sda_io_num = (gpio_num_t)DATA_IO;
    conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
    conf.scl_io_num = (gpio_num_t)CLK_IO;
...
{
    esp_log_level_set(AC101_TAG, ESP_LOG_DEBUG);

    if (InitI2C(I2C_MASTER_SDA_IO, I2C_MASTER_SCL_IO))
        return -1;
    esp_err_t res;
    res = WriteReg(CHIP_AUDIO_RS, 0x123);
    vTaskDelay(1000 / portTICK_PERIOD_MS);

    res &= 0x0101 == ReadReg(CHIP_AUDIO_RS);
    ...
    res |= SetI2sSampleRate(SAMPLE_RATE_44100);
    res |= SetI2sClock(BCLK_DIV_8, false, LRCK_DIV_32, false);
    res |= SetI2sMode(MODE_SLAVE);
    res |= SetI2sWordSize(WORD_SIZE_16_BITS);
...
{
    ESP_LOGI(AC101_TAG, "Dump AC101 Regs");
...
esp_err_t AC101::SetPAEnable(bool enable)
{
    ...
    if (enable)
    {
        result = gpio_set_level(GPIO_PA_EN, 1);
    }
    else
    {
        result = gpio_set_level(GPIO_PA_EN, 0);
    }
...
void AC101::callback(uint16_t event, void *param)
{
    switch (event)
    {
    case 0:
    ...
    case 1:
        break;
    default:
        ESP_LOGW(AC101_TAG, "Unhandled event %d", event);
        break;
    }
}
